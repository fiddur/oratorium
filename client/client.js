...
;((window) => {
  ...
  const renderDisplayNameSpan = ({ comment }) =>
    h('span', { class: 'commenter_name' }, comment.user.name || '')

  const renderCommentDiv = ({ comment }) =>
    h('div', { class: 'comment' }, [
  ...
  const renderPictureDiv = ({ user }) =>
    h(
      'div',
      { class: `comment_picture ${user.picture ? '' : 'unknown_user'}` },
      user.picture ? h('img', { src: user.picture || '', alt: user.name || '' }) : '?',
    )

  const renderCommentRow = ({ comment }) =>
    h('div', { class: 'comment_row', id: `comment_${comment.id}` }, [
      renderPictureDiv({ user: comment.user }),
      renderCommentDiv({ comment }),
    ])

  const renderCommentingDiv = ({ user, text, onInput, onSubmit }) =>
    h('div', { class: 'comment_row' }, [
  ...
  const openAuthWindow = (url) => () => window.open(url, 'authWindow', 'height=300px, width=500px')

  const renderLogin = ({ server, authenticators }) =>
    h('div', { class: 'login' }, [
  ...
  const putComment = ({ server, text, id, page }) =>
    fetch(`${server}/pages/${page}/comments/${id}`, {
  ...
  const Comments = (server) => (page) => {
    ...
    const mount = (element) => {
      ...
      store.addListener((newState) =>
        render(renderComments({ server, state: newState, commands: store.commands }), element, previous),
      )
